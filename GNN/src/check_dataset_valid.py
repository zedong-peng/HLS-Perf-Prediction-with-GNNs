...
def main():
    # 设置命令行参数解析
    parser = argparse.ArgumentParser(description='验证数据集完整性和有效性')
    parser.add_argument('--dataset_name', type=str, help='要检查的数据集名称')
    parser.add_argument('--max_samples', type=int, default=10000, 
                       help='统计节点数和边数时的最大样本数 (默认: 10000)')
    ...
    try:
        ...
        for i, data in enumerate(tqdm(dataset[:max_samples])):  # 限制样本数
            num_nodes_list.append(data.num_nodes)
            num_edges_list.append(data.num_edges)

        print("节点数统计:")
        print(f"  max: {np.max(num_nodes_list)}")
        print(f"  95%: {np.percentile(num_nodes_list, 95)}")
        print(f"  avg: {np.mean(num_nodes_list):.1f}")

        print("边数统计:")
        print(f"  max: {np.max(num_edges_list)}")
        print(f"  95%: {np.percentile(num_edges_list, 95)}")
        print(f"  avg: {np.mean(num_edges_list):.1f}")
