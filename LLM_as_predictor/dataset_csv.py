...
def base_feature(df):
    ...
    if kernel_rows.any():
        df.loc[kernel_rows, 'algo_name'] = df['File Path'].str.split('/').str[-6]
        df.loc[kernel_rows, 'source_name'] = df['File Path'].str.split('/').str[-7]
    ...
    if design_rows.any() and max_parts >= 8:
        ...
        try:
            ...
            df.loc[design_rows, 'algo_name'] = df['File Path'].str.split('/').str[-7]
            df.loc[design_rows, 'source_name'] = df['File Path'].str.split('/').str[-8]
        except IndexError:
